...
export default {
  inserted(el, binding) {
    ...
    // 绑定事件
    el.addEventListener('touchstart', data.activateTooltip, false);
    // cancel deactivate when enter
    el.addEventListener('touchstart', data.deactivateTooltip.cancel, false);
    el.addEventListener('touchend', data.deactivateTooltip, false);
    // cancel activate when leave
    el.addEventListener('touchend', data.activateTooltip.cancel, false);
  },
  ...
  unbind(el) {
    ...
    const data = getTooltipData(el);
    if (data) {
      el.removeEventListener('touchstart', data.activateTooltip, false);
      el.removeEventListener(
        'touchstart',
        data.deactivateTooltip.cancel,
        false
      );
      el.removeEventListener('touchend', data.deactivateTooltip, false);
      el.removeEventListener('touchend', data.activateTooltip.cancel, false);
